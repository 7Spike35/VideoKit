...
def search_bilibili(keywords, max_page, video_out_file, live_out_file=None):
    """
    根据关键词列表爬取 Bilibili 视频和直播数据，分别保存到 CSV 文件
    """
    # 将关键词列表合并为一个字符串（API 需要单个关键词）
    keyword = ' '.join(keywords)
    ...
    for page in range(1, max_page + 1):
        ...
        # 爬取视频
        video_url = 'https://api.bilibili.com/x/web-interface/search/type'
        video_params = {
            'search_type': 'video',
            'keyword': keyword,
            'page': page,
        ...
        try:
            r = requests.get(video_url, headers=headers, params=video_params)
            if r.status_code == 200:
                j_data = r.json()
                data_list = j_data['data']['result']
                print(f'视频数据长度: {len(data_list)}')
                ...
                print(f'第{page}页视频爬取完成。')
            else:
                print(f'视频请求失败，状态码：{r.status_code}')
        except Exception as e:
            print(f'视频发生错误: {e}')

        # 爬取直播
        if live_out_file:
            live_url = 'https://api.bilibili.com/x/web-interface/search/type'
            live_params = {'search_type': 'live', 'keyword': keyword, 'page': page, 'order': 'online'}
            try:
                response = requests.get(live_url, headers=headers, params=live_params)
                if response.status_code != 200:
                    print(f'直播请求失败，状态码：{response.status_code}')
                    continue
                response.encoding = 'utf-8'
                j_data = response.json()

                if j_data.get('code') != 0:
                    print(f'直播API返回错误：{j_data.get("message")}')
                    continue

                data = j_data.get('data', {})
                result = data.get('result', {})
                room_list = result.get('live_room', []) if isinstance(result, dict) else result if isinstance(result,
                                                                                                              list) else []
    ...
    if video_data:
        ...
        with open(video_out_file, 'w', encoding='utf-8-sig', newline='') as f:
            video_df.to_csv(f, index=False)
        print(f'视频数据已保存到{video_out_file}')

    # 保存直播数据
    if live_out_file and live_data:
        live_df = pd.DataFrame(live_data)
        live_df = live_df.sort_values(by='人气', ascending=False)  # 按人气降序排序
        with open(live_out_file, 'w', encoding='utf-8-sig', newline='') as f:
            live_df.to_csv(f, index=False)
        print(f'直播数据已保存到{live_out_file}')
